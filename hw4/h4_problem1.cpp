...
//定義平滑運算的次數
#define NSmooth 1000
#define wait(b) do{while(!(b))continue;}while(0)
...
//全域變數(data)，thread 共用 reference。
int nump=8;
pthread_mutex_t cnt_mux = PTHREAD_MUTEX_INITIALIZER;
int cnt[2]={0,0};

void barrier(bool state){
    pthread_mutex_lock(&cnt_mux);
    if(cnt[state]==(nump-1)){
        cnt[!state]=0;
        swap(BMPSaveData, BMPData);
        cnt[state]++;// 解鎖
        pthread_mutex_unlock(&cnt_mux);
    }else{
        cnt[state]++;
        pthread_mutex_unlock(&cnt_mux);
        wait(cnt[state]==nump);//等待解鎖
    }
}
...
void *thread_do(void* data)
{
    ...
    for (int count = 0; count < NSmooth; count++)
    {
        ...
        //進行平滑運算
        for (int i = myid; i < bmpInfo.biHeight; i+=nump){
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < nump; ++i) {
        ...
        *myid=i;
        pthread_create(&threads[i], NULL, thread_do, myid);
    }
    // main thread 等待...
    for (int i = 0; i < nump; ++i) {
    ...
    cout << "The execution time = "<<  elapsed <<endl ;
